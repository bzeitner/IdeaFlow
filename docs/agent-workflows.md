...
| Critique | Idea, PR, repository instructions, checks and prior comments | Spawn Principal Developer, Senior Developer, Security Architect, and Performance Developer reviewers; synthesize and post one evidence-based PR review; log review effort | Four role-specific reports, deduplicated prioritized findings, checks inspected, verdict | All four reports are collected, one review is posted, and the next action matches request-changes/comment/approve |
